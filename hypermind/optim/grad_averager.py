...
from hypermind.averaging.control import StepControl
from hypermind.dht import DHT
from hypermind.utils import DHTExpiration, get_logger
...
class GradientAverager(DecentralizedAverager):
    ...
    GradientAverager is meant to be used within hypermind.Optimizer, but it can be used standalone (see example below).
    ...
    :param dht: a DHT instance connected to the rest of the swarm. See hypermind.DHT docs
    ...
    >>> grad_averager = GradientAverager(model.parameters(), dht=hypermind.DHT(...))
    >>> next_step_time = hypermind.get_dht_time() + 60   # runs global steps every 60 seconds
    ...
    >>>    if next_step_time - hypermind.get_dht_time() <= 5
    ...
    >>>    if hypermind.get_dht_time() >= next_step_time:
    ...
    >>>        next_step_time = hypermind.get_dht_time() + 60
    >>>        next_step_control = None
