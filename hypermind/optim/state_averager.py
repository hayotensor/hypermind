...
from hypermind.compression import CompressionInfo, TensorRole
from hypermind.optim.grad_scaler import GradScaler
from hypermind.utils import DHTExpiration, PerformanceEMA, get_logger, nested_flatten, nested_pack
...
class TrainingStateAverager(DecentralizedAverager):
    ...
    :param custom_gradients: if True, do *not* automatically load local gradients into the offloaded optimizer.
      This assumes that offloaded gradients will be populated externally, e.g. by the user or by hypermind.Optimizer.
    ...
    def _init_components(
        ...
        if self.offload_optimizer and not optimizer_is_factory:
            raise ValueError("Using offload_optimizer requires creating optimizer inside hypermind")
    ...
    def step(
        ...
        :param grad_scaler: when using hypermind.GradScaler, one must forward it to step after calling .unscale_
