...
from hypermind.averaging.control import AveragingStage, StepControl
from hypermind.compression import CompressionBase, NoCompression
from hypermind.dht import DHT
from hypermind.optim.grad_averager import GradientAverager, GradientAveragerFactory
from hypermind.optim.grad_scaler import GradScaler
from hypermind.optim.progress_tracker import LocalTrainingProgress, ProgressTracker
from hypermind.optim.state_averager import (
...
from hypermind.utils import PerformanceEMA, get_dht_time, get_logger
...
class Optimizer(torch.optim.Optimizer):
    """
    hypermind.Optimizer wraps your regular PyTorch Optimizer for training collaboratively with peers.
    ...
    >>> dht = hypermind.DHT(initial_peers=INITIAL_PEERS, start=True)
    >>> opt = hypermind.Optimizer(dht=dht, run_id="run_42", batch_size_per_step=4, target_batch_size=4096,
    ...
    >>> dht = hypermind.DHT(initial_peers=INITIAL_PEERS, client_mode=IF_BEHIND_FIREWALL_OR_VERY_UNRELIABLE, start=True)
    >>> opt = hypermind.Optimizer(
    ...
    >>>
    >>>    grad_compression=hypermind.Float16Compression(),  state_averaging_compression=hypermind.Float16Compression(),
    >>>    # ^-- it is usually fine to use pure 16-bit or even lower precision during communication with no precaution;
    >>>    # See hypermind/examples/albert for an working example of mixed 8/16-bit compression.
    >>>
    ...
    :param dht: a running hypermind.DHT instance connected to other peers.
    ...
    :param optimizer: a callable(parameters) -> pytorch.optim.Optimizer or a pre-initialized PyTorch optimizer.
      **Note:** some advanced options like offload_optimizer, delay_optimizer_step, or delay_grad_averaging require
      and require the callable and will not work if hypermind.optimizer is created with a pre-existing PyTorch Optimizer.
    ...
    :param delay_state_averaging: if enabled (default), average parameters and extra tensors in a background thread;
      if set to False, average parameters synchronously within the corresponding hypermind.Optimizer.step call.
    ...
    :note: in a large-scale training, peers will inevitably fail and you will see error messages. hypermind.Optimizer
        ...
        if callable(optimizer) and params is not None:
            ...
            if offload_optimizer or delay_optimizer_step or delay_grad_averaging:
                raise ValueError(
                    ...
                    "hypermind.Optimizer(..., params=params, optimizer=lambda params: create_opt(params)"
                )
        else:
            raise ValueError(
                "Please initialize the optimizer in one of the following two ways:\n"
                "(A) hypermind.Optimizer(..., params=params, optimizer=lambda params: create_opt(params)\n"
                "(B) hypermind.Optimizer(..., optimizer=pre_initialize_optimizer)"
            )
    ...
    def step(
        ...
        :param grad_scaler: if amp is enabled, this **must** be a hypermind-aware gradient scaler.
        :note: this .step is different from normal pytorch optimizers in several key ways. See __init__ for details.
        """
        if grad_scaler is not None and not isinstance(grad_scaler, GradScaler):
            raise ValueError("hypermind.Optimizer requires a hypermind-aware gradient scaler (hypermind.GradScaler)")
