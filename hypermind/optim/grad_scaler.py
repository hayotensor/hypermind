...
class GradScaler(TorchGradScaler):
    """
    A wrapper over pytorch GradScaler made specifically for training hypermind.Optimizer with reuse_grad_buffers=True.
    ...
    hypermind.GradScaler makes 3 modifications to the regular PyTorch AMP:
    ...
    :note: The above modiffications will be enabled automatically. One can (and should) use hypermind.GradScaler exactly
    ...
    def unscale_(self, optimizer: TorchOptimizer) -> bool:
        with self._lock:
            assert isinstance(optimizer, hypermind.Optimizer)
    ...
    def step(self, optimizer: TorchOptimizer, *args, **kwargs) -> bool:
        if self._is_running_global_step and not isinstance(optimizer, hypermind.Optimizer):
            # ^-- invoked privately within hypermind optimizer
