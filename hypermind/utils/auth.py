...
from hypermind.proto.auth_pb2 import AccessToken, RequestAuthInfo, ResponseAuthInfo
...
from hypermind.substrate.chain_functions import is_subnet_node_by_a_parameter, is_subnet_node_by_peer_id
from hypermind.utils.timed_storage import TimedStorage, get_dht_time
...
class TokenAuthorizerBase(AuthorizerBase):
    """
    Implements the authorization protocol for a moderated Hypermind network.
...
class POSAuthorizer(AuthorizerBase):
    ...
    async def get_token(self) -> AccessToken:
        # Uses the built in Hypermind ``AccessToken`` format
        token = AccessToken(
            username='',
...
class POSAuthorizerLive(AuthorizerBase):
    ...
    def is_staked(self, peer_id_vector) -> bool:
        ...
        if not is_staked:
            is_staked = self.is_boostrap_node_staked(peer_id_vector)
    ...
    def is_boostrap_node_staked(self, peer_id_vector) -> bool:
