...
loglevel = os.getenv("HYPERMIND_LOGLEVEL", "INFO")

TRUE_CONSTANTS = ["TRUE", "1"]

_env_colors = os.getenv("HYPERMIND_COLORS")
if _env_colors is not None:
    use_colors = _env_colors.upper() in TRUE_CONSTANTS
else:
    use_colors = sys.stderr.isatty() or in_ipython()

_env_log_caller = os.getenv("HYPERMIND_ALWAYS_LOG_CALLER", "0")
always_log_caller = _env_log_caller.upper() in TRUE_CONSTANTS

enable_log_file = os.getenv("ENABLE_LOG_FILE", "false").lower() == "true"
...
_current_mode = HandlerMode.IN_HYPERMIND
...
def _initialize_if_necessary():
    ...
    with _init_lock:
        ...
        _enable_default_handler("hypermind")


def get_logger(name: Optional[str] = None) -> logging.Logger:
    """
    Same as ``logging.getLogger()`` but ensures that the default hypermind log handler is initialized.

    :note: By default, the hypermind log handler (that reads the ``HYPERMIND_LOGLEVEL`` env variable and uses
           the colored log formatter) is only applied to messages logged inside the hypermind package.
...
def use_hypermind_log_handler(where: Union[HandlerMode, str]) -> None:
    """
    Choose loggers where the default hypermind log handler is applied. Options for the ``where`` argument are:

    * "IN_HYPERMIND" (default): Use the hypermind log handler in the loggers of the ``hypermind`` package.
                               Don't propagate their messages to the root logger.
    * "nowhere": Don't use the hypermind log handler anywhere.
                 Propagate the ``hypermind`` messages to the root logger.
    * "in_root_logger": Use the hypermind log handler in the root logger
    ...
    if _current_mode == HandlerMode.IN_HYPERMIND:
        _disable_default_handler("hypermind")
