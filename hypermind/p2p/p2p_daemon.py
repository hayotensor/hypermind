...
import hypermind.hypermind_cli as cli
import hypermind.p2p.p2p_daemon_bindings.p2pclient as p2pclient
from hypermind.p2p.multiaddr import Multiaddr
from hypermind.p2p.p2p_daemon_bindings.control import DEFAULT_MAX_MSG_SIZE, P2PDaemonError, P2PHandlerError
from hypermind.p2p.p2p_daemon_bindings.datastructures import PeerID, PeerInfo, StreamInfo
from hypermind.p2p.p2p_daemon_bindings.utils import ControlFailure
from hypermind.proto import crypto_pb2
from hypermind.proto.p2pd_pb2 import RPCError
from hypermind.utils.asyncio import as_aiter, asingle
from hypermind.utils.crypto import Ed25519PrivateKey
from hypermind.utils.logging import get_logger, golog_level_to_python, loglevel, python_level_to_golog
...
class P2P:
    ...
    _UNIX_SOCKET_PREFIX = "/unix/tmp/hypermind-"
    ...
    async def create(
        ...
        *,
        ...
        if not all(arg is None for arg in [quic, use_relay_hop, use_relay_discovery]):
            warnings.warn(
                "Parameters `quic`, `use_relay_hop`, and `use_relay_discovery` of hypermind.P2P "
                "have no effect since libp2p 0.17.0 and will be removed in hypermind 1.2.0+",
                DeprecationWarning,
                stacklevel=2,
            )
