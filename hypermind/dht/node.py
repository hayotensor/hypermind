...
from hypermind.dht.crypto import DHTRecord, RecordValidatorBase
from hypermind.dht.protocol import DHTProtocol
from hypermind.dht.routing import DHTID, BinaryDHTValue, DHTKey, DHTValue, Subkey, get_dht_time
from hypermind.dht.storage import DictionaryDHTValue
from hypermind.dht.traverse import traverse_dht
from hypermind.p2p import P2P, PeerID
from hypermind.p2p.multiaddr import Multiaddr
from hypermind.utils import MSGPackSerializer, SerializerBase, get_logger
from hypermind.utils.auth import AuthorizerBase
from hypermind.utils.timed_storage import DHTExpiration, TimedStorage, ValueWithExpiration
...
class DHTNode:
    ...
    :note: Hypermind DHT is optimized to store a lot of temporary metadata that is regularly updated.
     For example, expert heartbeat emitted by a hypermind.moe.Server responsible for that expert.
    ...
    Similar to Kademlia RPC protocol, hypermind DHT has 3 RPCs:
    ...
    * find - request one or several keys, get values and expiration (if peer finds it locally) and :bucket_size: of
    ...
    async def create(
        ...
        **kwargs,
        ...
        """
        :param p2p: instance of hypermind.p2p.P2P that will be used for communication.
          If None, DHTNode will create and manage its own P2P instance with given initial_peers and
          parameters from ``kwargs``
        :param node_id: current node's DHTID for hypermind.dht, determines which keys it will store locally,
        ...
        :param kwargs: extra parameters for an internally created instance of hypermind.p2p.P2P.
        ...
        if p2p is None:
            ...
            if kwargs:
                raise ValueError(
                    f"**kwargs in DHTNode.create() should be empty if hypermind.p2p.P2P instance is provided"
