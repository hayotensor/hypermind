...
from hypermind.dht.routing import DHTID, BinaryDHTValue, RoutingTable, Subkey
from hypermind.dht.storage import DHTLocalStorage, DictionaryDHTValue
from hypermind.p2p import P2P, P2PContext, PeerID, ServicerBase
from hypermind.proto import dht_pb2
from hypermind.utils import MSGPackSerializer, get_logger
from hypermind.utils.auth import AuthorizerBase, AuthRole, AuthRPCWrapper
...
class DHTProtocol(ServicerBase):
    ...
    async def call_ping(self, peer: PeerID, validate: bool = False, strict: bool = True) -> Optional[DHTID]:
        ...
        """
        logger.debug(f"DHTProtocol call_ping, peer={peer}, validate={validate}, strict={strict}")
        ...
        logger.debug(f"DHTProtocol call_ping, responded={responded}")
    ...
    async def rpc_ping(self, request: dht_pb2.PingRequest, context: P2PContext) -> dht_pb2.PingResponse:
        ...
        logger.debug(f"DHTProtocol rpc_ping, response={response}")

        if request.peer and request.peer.node_id:
            sender_id = DHTID.from_bytes(request.peer.node_id)
            sender_peer_id = context.remote_id

            logger.debug(f"DHTProtocol rpc_ping, sender_peer_id={sender_peer_id}")

            if request.validate:
                logger.debug(f"DHTProtocol rpc_ping, request.validate={request.validate}, sender_peer_id={sender_peer_id}")
