...
from hypermind.dht.node import DEFAULT_NUM_WORKERS, DHTNode
from hypermind.dht.routing import DHTKey, DHTValue, Subkey
from hypermind.dht.validation import CompositeValidator, RecordValidatorBase
from hypermind.p2p import P2P, PeerID
from hypermind.p2p.multiaddr import Multiaddr
from hypermind.utils import MPFuture, get_logger, switch_to_uvloop
from hypermind.utils.timed_storage import DHTExpiration, ValueWithExpiration
...
class DHT(mp.context.ForkProcess):
    """
    A high-level interface to a hypermind DHT that runs a single DHT node in a background process.
    * hypermind servers periodically announce their experts via declare_experts (dht_handler.py)
    * trainers find most suitable experts via RemoteMixtureOfExperts (beam_search.py)
    ...
    :param kwargs: any other params will be forwarded to DHTNode and hypermind.p2p.P2P upon creation
    """
    ...
    def store(
        ...
        **kwargs,
        ...
        :param expiration_time: absolute time when the entry should expire, based on hypermind.get_dht_time()
