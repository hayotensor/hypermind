...
from hivemind.dht.routing_signature import DHTID, RoutingTable
...
def test_routing_table_basic():
    ...
    for phony_neighbor_port in random.sample(range(10000), 100):
        ...
        keypair = Keypair.create_from_uri("//{phony_neighbor_port}", crypto_type=KeypairType.SR25519)
        signature = keypair.sign(f"{LOCALHOST}:{phony_neighbor_port}")
