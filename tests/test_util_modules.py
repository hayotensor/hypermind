...
import hypermind
from hypermind.compression import deserialize_torch_tensor, serialize_torch_tensor
from hypermind.proto.runtime_pb2 import CompressionType
from hypermind.utils import BatchTensorDescriptor, DHTExpiration, HeapEntry, MSGPackSerializer, ValueWithExpiration
...
from hypermind.utils.mpfuture import InvalidStateError
...
async def test_await_mpfuture():
    ...
    f1, f2 = hypermind.MPFuture(), hypermind.MPFuture()
...
def test_mpfuture_bidirectional():
    evt = mp.Event()
    future_from_main = hypermind.MPFuture()

    def _future_creator():
        future_from_fork = hypermind.MPFuture()
    ...
    assert isinstance(out[1], hypermind.MPFuture)
...
def test_mpfuture_done_callback():
    ...
    def _future_creator():
        future1, future2, future3 = hypermind.MPFuture(), hypermind.MPFuture(), hypermind.MPFuture()
...
def test_many_futures():
    ...
    main_futures = [hypermind.MPFuture() for _ in range(1000)]
    assert len(hypermind.MPFuture._active_futures) == 1000

    def _run_peer():
        fork_futures = [hypermind.MPFuture() for _ in range(500)]
        assert len(hypermind.MPFuture._active_futures) == 500
        ...
        assert len(hypermind.MPFuture._active_futures) == 200
        for future in fork_futures:
            if not future.done():
                future.set_result(123)
        assert len(hypermind.MPFuture._active_futures) == 0
    ...
    assert len(hypermind.MPFuture._active_futures) == 700
...
def test_split_parts():
    ...
    chunks1 = list(hypermind.utils.split_for_streaming(serialized_tensor_part, 16384))
    assert len(chunks1) == int(np.ceil(tensor.numel() * tensor.element_size() / 16384))

    chunks2 = list(hypermind.utils.split_for_streaming(serialized_tensor_part, 10_000))
    assert len(chunks2) == int(np.ceil(tensor.numel() * tensor.element_size() / 10_000))

    chunks3 = list(hypermind.utils.split_for_streaming(serialized_tensor_part, 10**9))
    assert len(chunks3) == 1

    compressed_tensor_part = serialize_torch_tensor(tensor, CompressionType.FLOAT16, allow_inplace=False)
    chunks4 = list(hypermind.utils.split_for_streaming(compressed_tensor_part, 16384))
    assert len(chunks4) == int(np.ceil(tensor.numel() * 2 / 16384))

    combined1 = hypermind.utils.combine_from_streaming(chunks1)
    combined2 = hypermind.utils.combine_from_streaming(iter(chunks2))
    combined3 = hypermind.utils.combine_from_streaming(chunks3)
    combined4 = hypermind.utils.combine_from_streaming(chunks4)
    ...
    combined_incomplete = hypermind.utils.combine_from_streaming(chunks4[:5])
    combined_incomplete2 = hypermind.utils.combine_from_streaming(chunks4[:1])
    combined_incomplete3 = hypermind.utils.combine_from_streaming(chunks4[:-1])
    for combined in combined_incomplete, combined_incomplete2, combined_incomplete3:
        with pytest.raises(RuntimeError):
            deserialize_torch_tensor(combined)
            # note: we rely on this being RuntimeError in hypermind.averaging.allreduce.AllReduceRunner
