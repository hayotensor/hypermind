...
import hypermind
from hypermind.averaging.control import AveragingStage
from hypermind.optim.grad_averager import GradientAverager, GradientAveragerFactory
from hypermind.optim.optimizer import Optimizer
from hypermind.optim.power_sgd_averager import PowerSGDGradientAverager
from hypermind.optim.progress_tracker import ProgressTracker
from hypermind.optim.state_averager import ZERO_GRAD_SET_TO_NONE_DEFAULT, TrainingStateAverager
...
def test_grad_averager(grad_averager_factory: GradientAveragerFactory):
    ...
    dht2 = hypermind.DHT(start=True, initial_peers=dht1.get_visible_maddrs())
    ...
    control1 = averager1.schedule_step(hypermind.get_dht_time() + 5)
    control2 = averager2.schedule_step(hypermind.get_dht_time() + 5)
...
def test_state_averager(offload_optimizer: bool, reuse_tensors: bool, sync_epoch_when_averaging: bool):
    dht1 = hypermind.DHT(start=True)
    dht2 = hypermind.DHT(initial_peers=dht1.get_visible_maddrs(), start=True)
...
def test_progress_tracker():
    ...
    dht_root = hypermind.DHT(start=True)
    ...
    def run_worker(index: int, batch_size: int, period: float, **kwargs):
        dht = hypermind.DHT(initial_peers=dht_root.get_visible_maddrs(), start=True)
    ...
    local_epoch = 0
    last_timestamp = hypermind.get_dht_time()
    step_time_deltas = []
    ...
    while local_epoch < 6:
        ...
        if tracker.ready_to_update_epoch:
            ...
            time_delta = hypermind.get_dht_time() - last_timestamp
            if local_epoch == 2:
...
def _test_optimizer(
    ...
    def run_trainer(batch_size: int, batch_time: float, client_mode: bool):
        ...
        optimizer = Optimizer(
            ...
            dht=hypermind.DHT(initial_peers=dht.get_visible_maddrs(), client_mode=client_mode, start=True),
