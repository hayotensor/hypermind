...
from hypermind.proto import crypto_pb2, p2pd_pb2
...
async def test_dht_protocol():
    ...
    for client_mode in [True, False]:  # note: order matters, this test assumes that first run uses client mode
        ...
        store_ok = await protocol.call_store(peer1_id, [key], [hypermind.MSGPackSerializer.dumps(value)], expiration)
        ...
        recv_value = hypermind.MSGPackSerializer.loads(recv_value_bytes)
        ...
        assert await protocol.call_store(
            peer1_id,
            keys=[nested_key],
            values=[hypermind.MSGPackSerializer.dumps(value1)],
        ...
        assert await protocol.call_store(
            peer1_id,
            keys=[nested_key],
            values=[hypermind.MSGPackSerializer.dumps(value2)],
...
async def test_empty_table():
    ...
    assert all(await protocol.call_store(peer_peer_id, [key], [hypermind.MSGPackSerializer.dumps(value)], expiration))
