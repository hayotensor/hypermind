...
from hypermind.averaging.allreduce import AveragingMode
from hypermind.averaging.control import AveragingStage
from hypermind.averaging.key_manager import GroupKeyManager
from hypermind.averaging.load_balancing import load_balance_peers
from hypermind.averaging.partition import AllreduceException
...
def test_load_state_priority():
    ...
    averagers = []
    for i in range(4):
        averager = hypermind.DecentralizedAverager(
...
def test_averaging_trigger():
    ...
    for i, averager in enumerate(averagers):
        controls.append(
            averager.step(
                wait=False,
                scheduled_time=hypermind.get_dht_time() + 0.5,
                weight=1.0,
...
def test_averaging_cancel():
    ...
    step_controls = [averager.step(wait=False, scheduled_time=hypermind.get_dht_time() + 1) for averager in averagers]
